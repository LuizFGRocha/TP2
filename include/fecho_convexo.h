...
fecho_convexo varredura_de_graham(vetor<ponto> Q, int metodo_de_ordenacao) {
    ...
    std::cout << "Segundo ponto adicionado: ";
    vetor_ponto_angulo[0].primeiro().imprime_ponto();

    // Passo 6 do pseudocódigo
    if (vetor_ponto_angulo.size() >= 3) {
        S.empilha(vetor_ponto_angulo[1].primeiro());
        std::cout << "Terceiro ponto adicionado: ";
        vetor_ponto_angulo[1].primeiro().imprime_ponto();
    }
    ...
    for (int i = 2; i < vetor_ponto_angulo.size(); ++i) {
        ...
        while (nao_vira_para_a_esquerda(vetor_ponto_angulo[i].primeiro(), S.espia(), S.espia_segundo())) {
